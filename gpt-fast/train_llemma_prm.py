...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_args, tokenizer: transformers.PreTrainedTokenizer):
        ...
        if 'instruction' in list_data_dict[0]:
            ...
            list_data_dict = [{'input':data['input'], 'output':data['output'], 'is_eos': data['is_eos']} for data in list_data_dict]
        ...
        for example in list_data_dict:
            if example['is_eos']:
                target_string = f"{example['output']}{tokenizer.eos_token}"
            else:
                target_string = example['output']
...
def train():
    ...
    print('max token length:', training_args.model_max_length)
    if model_args.model_name_or_path == 'EleutherAI/llemma_7b' or model_args.model_name_or_path == 'EleutherAI/llemma_34b':
    ...
    trainer.train()
    trainer.save_state()
    model.save_pretrained(training_args.output_dir+'_bin', safe_serialization=False)

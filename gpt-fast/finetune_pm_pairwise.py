...
def model_forward_with_loss(
    ...
    assert choice.size(0) == B

    device = input_ids.device
    with torch.device(device):
        model.backbone_model.setup_caches(
            max_batch_size=B, max_seq_length=T, kv_cache=False
        )
    ...
    with torch.backends.cuda.sdp_kernel(
        ...
        logits = model_forward(model, input_ids, eos_pos)
...
def main(
    ...
    rm_checkpoint_path = args.rm_checkpoint_path
    ...
    if rm_checkpoint_path is not None:
        assert rm_checkpoint_path.is_dir(), rm_checkpoint_path

    assert not (
        sft_checkpoint_path is not None and rm_checkpoint_path is not None
    ), "Cannot specify both sft_checkpoint_path and rm_checkpoint_path"

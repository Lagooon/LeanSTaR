...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --standalone --nproc_per_node=8 \
    ...
    --learning_rate 2e-5 \
    --num_train_epochs 2 \
    --dataset "data/leandojo_benchmark_4/processed/gpt4-generated-train-plus.json" \
    --dataset_format "mapped" \
    --add_eos_to_target \
    --save_strategy epoch \
    --save_steps 60\
    --save_total_limit 1 \
    --save_dir $DATA_DIR/checkpoints/internlm2-7b_cot_epoch-1_lr-3e-5-plus \
    --sft_checkpoint_path $DATA_DIR/checkpoints/internlm2-7b_sft_epoch-1_lr-3e-5-plus \

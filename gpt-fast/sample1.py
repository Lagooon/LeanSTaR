...
def _load_model(checkpoint_path, device, precision, tp_size):
    model_name = "/data/user_data/shengyuf/Lean/checkpoints/internlm/internlm2-math-plus-7b/sft"
    '''
    engine_args = vllm.AsyncEngineArgs(
        model=model_name,
        tensor_parallel_size=tp_size,
        engine_use_ray=bool(tp_size > 1),
        worker_use_ray=bool(tp_size > 1),
        dtype=precision,
        max_num_batched_tokens=32768,
        trust_remote_code=True,
        enforce_eager=True,
    )
    model = vllm.AsyncLLMEngine.from_engine_args(engine_args)
    '''
    model = vllm.LLM(
        model=model_name,
        tensor_parallel_size=tp_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clear-process-hours', type=int, default=30)

...
def decode_n_tokens(
    ...
    **sampling_kwargs,
    ...
    for i in range(num_new_tokens):
        ...
        if eos_flag is not None:
            eos_flag = eos_flag | (next_token == tokenizer.eos_id)
...
def generate_tactic(
    ...
    with torch.device(device):
        model.setup_caches(max_batch_size=batch_size, max_seq_length=1024, kv_cache=True)
    full_y_list = None
    if not prompted:
        prompts = [_prompt(prompt) for prompt in prompts]
    encoded, left_pad_mask_pos = batch_encode_tokens(
                    tokenizer, prompts, bos=True, device=device, max_length=896
                )
    ...
    for i in range(0, num_samples, batch_size):
        y, num_decoded_tokens = generate(
                model,
                encoded[i:i+batch_size],
                left_pad_mask_pos[i:i+batch_size] if left_pad_mask_pos != None else None,
                max_seq_len,
    ...
    try:
        with Dojo(theorem, hard_timeout=timeout) as (dojo, init_state):
            ...
            for iteration in trange(max_iters):
                ...
                ts = [_tactic_state(state[0]) for state in states]
                ...
                for i in range(num_samples):
                    step, state = step_cands[i], states[i]
                    state, steps, trace = state
                    ...
                    if isinstance(result, ProofFinished):
                        ...
                        states[i] = (result, steps+[step], trace+[step_trace])

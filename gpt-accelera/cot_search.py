...
def _load_model(checkpoint_path, device, precision, tp_size):
    model_name = "/nobackup/users/zhiqings/haohanl/Lean/checkpoints/internlm/internlm2-math-base-7b/cot"
    model = vllm.LLM(
        model=model_name,
...
def generate_tactic(
    ...
    outputs = outputs + model.generate([prompt], params, use_tqdm=False)
    if len(outputs) == 0:
        return [], []
    ...
    try:
        with Dojo(theorem, hard_timeout=timeout) as (dojo, init_state):
            ...
            for iteration in trange(max_iters):
                ...
                total_score, _, steps, state, trace = heapq.heappop(queue)
                ts = _tactic_state(state)
                visited.add(ts)
                
                step_cands_cot, step_scores = generate_tactic(
                    ts,
                    model,
                    tokenizer,
                ...
                #if iteration < 2:
                #    print(iteration, " # state: ",state)
                #    print("tatics: ", step_cands[0])

                step_cands = [s.split("```lean4\n")[-1].split('```')[0].strip() for s in step_cands_cot]

                for step, score, step_cot in zip(step_cands, step_scores, step_cands_cot):
...
def main(
    ...
    #data = data[(320+765+250+2190+1179+1375):]

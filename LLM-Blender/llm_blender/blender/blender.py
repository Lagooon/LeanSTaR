...
class Blender:
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                if self.ranker_config.ranker_type == "pairranker":
                    ...
                    batch_scores = get_scores_from_cmps(preds, policy=policy)
    ...
    def compare_conversations(
        ...
        for c in conversations_a + conversations_b:
            ...
            assert all(
                [c[i]["role"] == "USER" for i in range(0, len(c), 2)]
            ), "Each even turn must be USER"
            assert all(
                [c[i]["role"] == "ASSISTANT" for i in range(1, len(c), 2)]
            ), "Each odd turn must be ASSISTANT"
        # check conversations correctness
        assert len(conversations_a) == len(
            conversations_b
    ...
    def n_generate(
        ...
        **generate_kwargs: dict,
        ...
        sampled_candidates: List[List[str]] = (
            []
